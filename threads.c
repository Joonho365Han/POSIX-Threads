...
#define THREAD_COUNT_LIMIT 88000
...
/* ::: START Helper functions & shared variables */
...
TCB 		*__tcb_list[THREAD_COUNT_LIMIT]; //  Array of TCB pointers.
int 		__tcb_extra = 0; //  Number of threads that are not MAIN.
pthread_t 	__curr_t_id = 0; //  The running thread's ID
pthread_t	__new_tcbid = 0; //  New thread ID to be assigned
sem_t		__pthread_mutex; //  Mutex for tcb-related variable access
...
void __usermode_scheduler(int alarm)
{
	//  Change states of threads
	lock();
	if (__tcb_list[(int)__curr_t_id]->state == THREAD_RUNNING)
		__tcb_list[(int)__curr_t_id]->state = THREAD_READY;
	else if (__tcb_list[(int)__curr_t_id]->state == THREAD_EXITED)
		while (__tcb_list[(int)__curr_t_id]->listener!=NULL)
		{
			__tcb_list[(int)__curr_t_id]->first_in_line = __tcb_list[(int)__curr_t_id]->listener;
			__tcb_list[__tcb_list[(int)__curr_t_id]->first_in_line->id]->state = THREAD_READY;
			__tcb_list[(int)__curr_t_id]->listener = __tcb_list[(int)__curr_t_id]->first_in_line->next;
			free(__tcb_list[(int)__curr_t_id]->first_in_line);
		}

	//  Save state of thread OR return to thread execution.
	if (setjmp(&__tcb_list[(int)__curr_t_id]->jmp_buf)) 
		return;

	//  Find ready thread other than current one
	do {
		__curr_t_id = (pthread_t)(((int)__curr_t_id+1) % (int)__new_tcbid);
	}
	while (__tcb_list[(int)__curr_t_id]->state != THREAD_READY);
	__tcb_list[(int)__curr_t_id]->state = THREAD_RUNNING;

	//  Reactive scheduler only when there are other threads to switch into.
	if (__tcb_extra) unlock();

	//  Execute new thread
	longjmp(&__tcb_list[(int)__curr_t_id]->jmp_buf, 1);
}
...
int sem_wait(sem_t *sem)
{
	...
	if (r_sem->value == 0)
	{
		__tcb_list[(int)__curr_t_id]->state = THREAD_BLOCKED;
...
int sem_post(sem_t *sem)
{
	...
	if (r_sem->listener != NULL)
	{
		...
		__tcb_list[(int)listener->id]->state = THREAD_READY;
...
int pthread_create(
	...
	void *arg)
{
	if ((int)__new_tcbid == 0) // No threads in TCB list (not even the main thread)
	{	
		//  Initialize pthread library semaphore
		sem_init(&__pthread_mutex, 0, 1);
	...
	sem_wait(&__pthread_mutex);
	if (__new_tcbid == THREAD_COUNT_LIMIT)
	{
		sem_post(&__pthread_mutex);
		return -1;
	}
	__tcb_list[(int)__new_tcbid] = (TCB*) malloc(sizeof(TCB));
	TCB *new_tcb = __tcb_list[(int)__new_tcbid];
...
int pthread_join(pthread_t thread, void **value_ptr)
{
	...
	sem_wait(&__pthread_mutex);
	if (__tcb_list[(int)thread]->state != THREAD_EXITED)
	{
		__tcb_list[(int)__curr_t_id]->state = THREAD_BLOCKED;
		__listener *new_head = (__listener*) malloc(sizeof(__listener));
		new_head->id = __curr_t_id;
		new_head->next = __tcb_list[(int)thread]->listener;
		__tcb_list[(int)thread]->listener = new_head;
		sem_post(&__pthread_mutex);
		__usermode_scheduler(0);
	}
	else
	...
	if (value_ptr != NULL)
		*value_ptr = __tcb_list[(int)thread]->exitval;
...
void pthread_exit(void *value_ptr) //  This function cannot have any local variables as there will be no stack 
{
	...
	sem_wait(&__pthread_mutex);
	free(__tcb_list[(int)__curr_t_id]->t_stack);
	__tcb_list[(int)__curr_t_id]->state = THREAD_EXITED;
	__tcb_list[(int)__curr_t_id]->exitval = value_ptr;
	--__tcb_extra;

	//  Execute, but not call, the scheduler.
	sem_post(&__pthread_mutex);
	setjmp(&__tcb_list[(int)__curr_t_id]->jmp_buf);
	__tcb_list[(int)__curr_t_id]->jmp_buf.__jmpbuf[JB_PC] = __ptr_mangle((int) __usermode_scheduler);
	longjmp(&__tcb_list[(int)__curr_t_id]->jmp_buf, 0);
}
